...
export async function POST(req: NextRequest) {
  try {
    ...
    const existingMunicipalities = await db.municipality.count()
    ...
    if (existingMunicipalities === 0) {
      ...
      return NextResponse.json({
        success: true,
        message: 'Datos geográficos de Colombia inicializados exitosamente',
        data: {
    ...
    // If both exist, just return status
    return NextResponse.json({
      success: true,
      message: 'Datos geográficos ya inicializados',
      data: {
        departments: departments.length,
    ...
    console.error('Error initializing Colombia data:', error)
    return NextResponse.json({
      error: 'Error del servidor',
      message: 'Ocurrió un error al inicializar los datos geográficos'
    }, { status: 500 })
  }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const municipalities = await db.municipality.count()
    ...
    return NextResponse.json({
      ...
      data: {
        departments,
        municipalities,
        initialized: departments > 0 && municipalities > 0
      }
    })
    ...
    return NextResponse.json({
      error: 'Error del servidor',
      message: 'Ocurrió un error al verificar el estado de los datos'
    }, { status: 500 })
  }
}
