...
export default function PriorityReportPage() {
  ...
  const fetchReport = async (leaderId: string) => {
    try {
      setIsLoading(true)
      const response = await fetch(`/api/dashboard/leader/witnesses/priority-report?leaderId=${leaderId}`)
      if (response.ok) {
    ...
    <div className="min-h-screen flex flex-col bg-muted/10">
      ...
      <main className="container mx-auto px-4 py-6 flex-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          ...
          />
          <StatCard
            title="Votantes Sin Cobertura"
            value={report.statistics.votersWithoutWitness}
            icon={TrendingDown}
            description="Sin testigo asignado"
            trend="down"
          />
        </div>
